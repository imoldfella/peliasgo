#this shows how I edited the postgis install for a mac

TMPDIR=~/gisdata/temp/
UNZIPTOOL=unzip
WGETTOOL="/usr/bin/wget"
export PGBIN=/opt/homebrew/bin
export PGPORT=5432
export PGHOST=localhost
export PGUSER=jim
export PGDATABASE=jim
PSQL=${PGBIN}/psql
SHP2PGSQL=shp2pgsql
cd ~/gisdata

cd ~/gisdata
wget https://www2.census.gov/geo/tiger/TIGER2021/STATE/tl_2021_us_state.zip --mirror --reject=html
cd ~/gisdata/www2.census.gov/geo/tiger/TIGER2021/STATE
rm -f ${TMPDIR}/*.*
...
${SHP2PGSQL} -D -c -s 4269 -g the_geom   -W "latin1" tl_2021_us_state.dbf tiger_staging.state | ${PSQL}
...
cd ~/gisdata
wget https://www2.census.gov/geo/tiger/TIGER2021/COUNTY/tl_2021_us_county.zip --mirror --reject=html
cd ~/gisdata/www2.census.gov/geo/tiger/TIGER2021/COUNTY
rm -f ${TMPDIR}/*.*
${PSQL} -c "DROP SCHEMA IF EXISTS tiger_staging CASCADE;"
${PSQL} -c "CREATE SCHEMA tiger_staging;"
...
${SHP2PGSQL} -D -c -s 4269 -g the_geom   -W "latin1" tl_2021_us_county.dbf tiger_staging.county | ${PSQL}
