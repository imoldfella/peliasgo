...
type TileSource interface {
	Get(zoom, x, y int) ([]byte, error)
	GetMetadata() string
}

// +"/output.mbtiles"

func httpString(s string) func(w http.ResponseWriter, r *http.Request) {
	return func(w http.ResponseWriter, r *http.Request) {
		w.Write([]byte(s))
	}
}

// path is for index.html other things should be transitioned.
// even index.html could probably be a template for our purposes.
func ServeTiles(src TileSource, port string, path string) error {

	getTile := func(w http.ResponseWriter, r *http.Request) {
		vars := mux.Vars(r)
	...
	{
		x := src.GetMetadata()
		var meta encode.Metadata
		json.Unmarshal([]byte(x), &meta)
		meta.Tiles = []string{
			// will this default back to me?
	...
	r.HandleFunc("/rlp/{z}/{x}/{y}.pbf", getTile)
	r.HandleFunc("/rlp.json", httpString(style))
	r.PathPrefix("/fonts").Handler(http.FileServer(http.Dir(path)))
	r.HandleFunc("/", httpString(index_html))

	// the server needs to alter the meta data in order to point to the getTile function reliably?
	r.HandleFunc("/metadata.json", httpString(string(metadata)))
